...
png("plot2.png", width = 480, height = 480) #creating a png device of the required height and width specifications
Line_Graph <- plot(EPCS$Global_active_power~EPCS$Date_Time, type="l", xlab="", ylab="Global Active Power (kilowatts)") # Creating line graph of Global active power use vs the date and time, the type function creates a line graph while xlab="" removes the x axis label and ylab sets the label on the y axis
